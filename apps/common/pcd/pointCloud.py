...
class PointCloudFrame:
    def __init__(self, seq: int, ts: int, point_cloud: SimplePointCloud5D):
        self.seq = seq
        self.ts = ts
    ...
    def serialize(self, compact=False)-> Union[dict, List]:
        if compact:
            return [self.seq, self.ts, self.point_cloud.serialize(compact)]
        else:
            return {'seq': self.seq, 'ts': self.ts, 'points': self.point_cloud.serialize(compact)}