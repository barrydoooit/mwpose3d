...
class SkeletonKeypointFilter(BaseTransform):
    def __init__(self, keypoints_involved: List[int], online_mode: bool = False):
...
class SkeletonCoordNormalization(BaseTransform):
    def __init__(
        self, means: List[float], stds: List[float], online_mode: bool = False
    ):
        super().__init__(online_mode)
...
class SkeletonDatasetTransform(BaseTransform):
    ...
    def transform(self, input: dict[str, any]) -> dict[str, any]:
        ...
        assert (
            len(data) == 1
        ), f"This is likely more than one batch, which is not supported: {data}"

        # Need to massage the data a bit on order to reorder nicely
        # Place this in a for loop if we need to support batching
        data = np.array(data).reshape((-1, 3))
        data = self.reorder_axis(data, self.mapping)
